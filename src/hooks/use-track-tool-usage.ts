...
const toolInfo: Record<string, { name: string, icon: string }> = {
  ...
  '/password-generator': { 
    ...
    icon: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="7.5" cy="15.5" r="5.5"></circle><path d="m21 2-9.6 9.6"></path><path d="m15.5 7.5 3 3L22 7l-3-3"></path></svg>' 
  },
  '/qr-code-generator': { 
  ...
  '/text-diff': { 
    name: 'Text Diff', 
    icon: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v14"></path><path d="M5 10h14"></path><path d="M5 21h14"></path></svg>' 
  },
  '/code-formatter': { 
    name: 'Code Formatter', 
    icon: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="16 18 22 12 16 6"></polyline><polyline points="8 6 2 12 8 18"></polyline></svg>' 
...
export function useTrackToolUsage() {
  ...
  useEffect(() => {
    ...
    if (pathname && toolInfo[pathname]) {
      const { name, icon } = toolInfo[pathname]
      ...
      try {
        ...
        let recentTools = storedRecent ? JSON.parse(storedRecent) : []
        
        // Remove if already exists
        recentTools = recentTools.filter((tool: any) => tool.href !== pathname)
        
        // Add to beginning and limit to 5 items
        recentTools = [{ name, href: pathname, icon }, ...recentTools].slice(0, 5)
        
        // Save back to localStorage
        localStorage.setItem('recent-tools', JSON.stringify(recentTools))
      } catch (error) {
        console.error('Error storing recent tools in localStorage:', error)
      }
    }
